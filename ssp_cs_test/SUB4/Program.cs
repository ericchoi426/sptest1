...
namespace SP_TEST
{
    class Program
    {
        ...
        public class Worker
        {
            ...
            public bool enc_data(string file_name, ref List<string> results,int count = 0)
            {
            ...
            public void DoWork()
            {
                ...
                try
                {
                    ...
                    while (true)
                    {
                        ...
                        {
                            int index = 0;
                            var str = System.Text.Encoding.Default.GetString(recvbuf);
                            str = str.Trim('\0');
                            List<string> out_data = new List<string>();
                            if(enc_data(str,ref out_data))
                            {
                            ...
                            while ((recvLen = handler.Receive(recvbuf)) > 0)
                            {
                                ...
                                else
                                {
                                    ...
                                    out_data.Clear();
                                    if (enc_data(str, ref out_data,num))
                                    {
                                        index = 0;
                                        byte[] byData = System.Text.Encoding.ASCII.GetBytes(out_data[index]);
                                        handler.Send(byData);
